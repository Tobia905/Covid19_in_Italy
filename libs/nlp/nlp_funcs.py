...
def counter_from_dict(vocab):
    
    if not isinstance(vocab, dict):
        raise TypeError("Vocab should be a dictionary")
        
    bigs = [str(big)[1:] for big in vocab.keys()]
    cnt = [c for c in vocab.values()]
    big_count = pd.DataFrame({'big':bigs, 
                              'count':cnt}).sort_values(by = 'count', ascending = False)
    
    return big_count

def counter_from_list(vocab):
    return pd.DataFrame(vocab).rename({0:'Word', 1:'Count'}, axis = 1).set_index('Word')

def plot_counter(vocab, count_lim = None, ax = None, title = 'Word Count'):
    
    if isinstance(vocab, dict):
        wc = counter_from_dict(vocab)
    elif isinstance (vocab, list):
        wc = counter_from_list(vocab)
    elif isinstance(vocab, pd.core.frame.DataFrame):
        wc = vocab.set_index('Word')
        
    if count_lim is not None:
        wc = wc[wc.Count >= count_lim]
        
    wc.plot.bar(ax = ax, legend = False)
    _ = ax.grid(alpha=.3)
    _ = ax.set_xlabel('')
    _ = ax.set_ylabel('#')
    _ = ax.set_title(title)
    _ = ax.set_xticklabels(wc.index, rotation = 45, ha = 'right')
