...
from modeling.get_best_model import _score_renamer, smote_cv_score, parallel_cv_process, _check_parallel
from data_management.data_utilities import _get_mapper
...
def plot_confusion_matrix(y_test,
    ...
    if cv is not None:
        cms = smote_cv_score(x, y, model = model, cv = cv, imbalanced = imbalanced, 
                             strategy = strategy, under_strategy = under_strategy, 
                             random_state = random_state, scoring = 'confusion_matrix', method = method)
...
def CV_roc_curve(x,
    ...
    int_tpr_mean = []
    rocs = smote_cv_score(x, y, model = model, cv = cv, imbalanced = imbalanced, 
                          strategy = strategy, under_strategy = under_strategy, 
                          random_state = random_state, scoring = 'roc_curve', method = method)
    aucs = smote_cv_score(x, y, model = model, cv = cv, imbalanced = imbalanced, 
                          strategy = strategy, under_strategy = under_strategy, 
                          random_state = random_state, scoring = 'roc_auc', method = method)
...
def CV_precision_recall(x,
    ...
    int_pre_mean = []
    prec = smote_cv_score(x, y, model = model, cv = cv, imbalanced = imbalanced, 
                          strategy = strategy, under_strategy = under_strategy, 
                          random_state = random_state, scoring = 'precision_recall_curve', method = method)
