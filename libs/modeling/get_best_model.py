...
def _get_estimator(pipeline):
    return [
        mod for mod in pipeline if not hasattr(mod, 'fit_transform') 
        and not hasattr(mod, 'fit_sample') and not hasattr(mod, 'fit_resample')
    ][0]
...
def _extract_estimator_from_composite(pipeline):
    ...
    for i in comp_models:
        estimators.append(_get_estimator(i))
...
def get_report_df(rep, composite = False, be = False):
    ...
    if composite:
        ...
        rep = _switch_keys(rep, ml_models)
...
def _score_mapper(y_test, pred, prob = None, model = None, pos_label = 1):
    ...
    def get_fimp(model):
        if isinstance(model, Pipeline) or isinstance(model, imPipeline):
            model = [mod for mod in model 
                     if not hasattr(mod, 'fit_transform') and not hasattr(mod, 'fit_sample')][0]
        try:
...
def parallel_cv_process(
    ...
    out = Parallel(
        ...
        pre_dispatch = pre_dispatch)(
            delayed(_train_cv_for_parallel)(
                X, Y, model, train_index,
...
def _check_parallel(
    ...
    return _cv_score, cv_args

def standard_classifier_builder(random_state = 10, no_conv_risk = False):
...
def cross_val_report(
    ...
    _cv_score, cv_args = _check_parallel(
            n_jobs = n_jobs,
            pre_dispatch = pre_dispatch,
            imbalanced = imbalanced,
            strategy = strategy,
    ...
    for mod in models:
        rep = {}
        for cl in classes: 
            rep[cl] = {score:_cv_score(x, y, scoring = score,
                                       random_state = random_state,
                                       model = mod, cv = cv, 
                                       pos_label=cl, stratified = stratified,
                                       shuffle = shuffle, 
                                       **cv_args) for score in scores}
            if mean_cv:
                for score in scores:
                    if score == 'support':
        ...
        for avg, avg_score in zip(avgs, [macro_scores, micro_scores, weighted_scores]):
            rep[avg] = {score.split('_')[0]:_cv_score(x, y, 
                                                      scoring = score, 
                                                      random_state = random_state,
                                                      model = mod, 
                                                      cv = cv,
                                                      ...
                                                      shuffle = shuffle,
                                                      **cv_args) for score in avg_score}
            
            rep['accuracy'] = _cv_score(x, y, scoring = 'accuracy',
                                        random_state = random_state, 
                                        model = mod, 
                                        cv = cv,
                                        stratified = stratified,
...
def refit_by_importance(
    ...
    if isinstance(models[0], Pipeline) or isinstance(models[0], imPipeline):
        composite = True
    ...
    smote_pars = {}
    smote_pars_to_check = ['imbalanced', 'strategy', 'under_strategy']
    for sp in smote_pars_to_check:
        if sp in kwargs.keys():
            smote_pars[sp] = kwargs[sp] 
        else:
            smote_pars[sp] = None
    
    cpkwargs = kwargs
    if 'imbalanced' in cpkwargs.keys():
        del cpkwargs['imbalanced'] 
        
    elif 'strategy' in cpkwargs.keys():
        del cpkwargs['strategy'] 
        
    elif 'under_strategy' in cpkwargs.keys():
        del cpkwargs['under_strategy']
    
    _cv_score, cv_args = _check_parallel(n_jobs = n_jobs,
                                         pre_dispatch = pre_dispatch,
                                         **smote_pars,
                                         **cpkwargs)
    if 'imbalanced' in cv_args:
        cv_args['scale'] = scale
        
    f_kwargs = kwargs
    if 'normalize' in f_kwargs.keys():
        del f_kwargs['normalize']
        
    cv_or_test = {'cross_val': cross_val_report, 'test': get_model_score}
    fimps = [np.mean(np.array(_cv_score(x, y, model = mod, 
                              scoring = 'feature_importance',
                              random_state = random_state,
                              **cv_pars, **cv_args)), axis = 0) for mod in models]
...
def importance_parser(
    ...
    ml_models = _extract_estimator_from_composite(selector)
    ml_models = list(map(lambda mod: mod.__class__.__name__, ml_models))
    rep = _switch_keys(selector, ml_models)
    
    subset = {}
    for mod in rep.keys():
        subset[mod] = {score: {imp:rep[mod][imp][0][measure][score]
                               for imp in rep[mod].keys()
                               } 
                       for score in scores
                       }
    if to_table:
        idx_1 = list(subset.keys())
        get = scores[0]
        ranges = [
            list(list(subset.values())[sc][get].keys())
            for sc in range(len(list(subset.values())))
        ]
        idx_1 = [[ind] * len(ranges[n]) for n,ind in enumerate(idx_1)]
        idx_1 = list(itertools.chain(*idx_1))
        ...
        for sc in scores:
            all_scores.append(list(
                itertools.chain(*[
                    list(list(subset.values())[n][sc].values())
                         for n, _ in enumerate(list(subset.values()))
                    ]
                )
            )
        )
        ...
        final_parsed = np.hstack((np.array([idx_1, ranges]).transpose(), all_scores))
        
        subset = pd.DataFrame(final_parsed, 
                              columns = ['Model', 'Treshold'] + scores).set_index(['Model', 'Treshold'])
        
    if max_val is not None:
        subset = subset[subset[max_val] == subset[max_val].max()]
    ...
    if print_measure:
        print(f'Parsed Results for {label}{measure} Scores:')
    
    return subset

def optimal_treshold(parsed_df, to_table=True, **kwargs):
    if isinstance(parsed_df, dict):
        parsed_df = importance_parser(parsed_df, to_table=to_table, **kwargs)

    if isinstance(parsed_df, pd.core.frame.DataFrame) and parsed_df.shape[0] != 1:
        assert ('max_val' in kwargs)
        max_val = kwargs['max_val']
        parsed_df = parsed_df[parsed_df[max_val] == parsed_df[max_val].max()]

    idx = parsed_df.index.tolist()[0]
